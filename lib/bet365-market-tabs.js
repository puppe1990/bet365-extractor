...
export const MARKET_TAB_VISIT_BUDGET_MS = 35_000;
...
export const PREMATCH_MARKET_TAB_BAND_TOP_PX = 720;
export const PREMATCH_MARKET_TAB_BAND_TOP_RATIO = 0.72;
export const PREMATCH_MARKET_TAB_CONTAINER_MIN_SCORE = 2;
...
export function marketTabContainerMinScore(pageMode = "auto") {
  return pageMode === "prematch" ? PREMATCH_MARKET_TAB_CONTAINER_MIN_SCORE : 3;
}

export function isExactMarketTabLabel(text, label) {
  return new RegExp(`^${escapeRegExp(label)}$`, "i").test(normalizeMarketTabLabel(text));
}

export function pickMarketTabNodesByLabel(nodes, visitList = []) {
