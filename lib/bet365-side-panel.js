import { isLikelyMinuteAsOdd } from "./bet365-parsers.js";
...
export function enrichGoalHint(hint = {}, events = [], texts = []) {
...
export function isRealTimelineGoal(event) {
...
export function stripGoalAbsorbedTimelineEvents(events) {
...
export function extractLineupSectionFromText(text) {
  const lines = linesFromText(text);
...
export function extractGoalScorersSectionFromText(text) {
  const lines = linesFromText(text);
  for (let i = 0; i < lines.length; i++) {
...
export function parseLineupFromTitularesText(text, options = {}) {
  const lines = linesFromText(text);
